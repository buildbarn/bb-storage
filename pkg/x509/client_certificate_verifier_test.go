...
var (
	...
	// openssl req -x509 -sha256 -nodes -days 365 -newkey rsa:4096 -keyout key -out crt -extensions extensions -config client_certificate_verifier_valid.cnf
...
func TestClientCertificateVerifier(t *testing.T) {
	...
	aVerifier := bb_x509.NewClientCertificateVerifier(clientCAs, clock, aValidator, metadataExtractor)
	bVerifier := bb_x509.NewClientCertificateVerifier(clientCAs, clock, bValidator, metadataExtractor)

	t.Run("NoCertificateProvided", func(t *testing.T) {
		// Connection with no certificate provided by the client.
		_, err := aVerifier.VerifyClientCertificate(nil)
	...
	t.Run("NoCAMatch", func(t *testing.T) {
		...
		_, err := aVerifier.VerifyClientCertificate(
			[]*x509.Certificate{
	...
	t.Run("ValidationFail", func(t *testing.T) {
		...
		_, err := bVerifier.VerifyClientCertificate(
			[]*x509.Certificate{
				certificateValid,
			},
		)
	...
	t.Run("Success", func(t *testing.T) {
		...
		clock.EXPECT().Now().Return(time.Unix(1712700000, 0))
		actualMetadata, err := aVerifier.VerifyClientCertificate(
			[]*x509.Certificate{
				certificateValid,
			},
		)
