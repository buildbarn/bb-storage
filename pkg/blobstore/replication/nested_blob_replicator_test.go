...
func TestNestedBlobReplicator(t *testing.T) {
	...
	treeReader := mock.NewMockBlobStreamReader(ctrl)
	...
	t.Run("Example", func(t *testing.T) {
		...
		replicator.EXPECT().ReplicateMultiple(ctx, actionDigest.ToSingletonSet()).Return(nil)
		actionReader.EXPECT().ReadMessage(ctx, actionDigest).Return(action, nil)
		replicator.EXPECT().ReplicateMultiple(ctx, commandDigest.ToSingletonSet()).Return(nil)

		replicator.EXPECT().ReplicateMultiple(ctx, fullDirectoryDigest.ToSingletonSet()).Return(nil)
		directoryReader.EXPECT().ReadMessage(ctx, fullDirectoryDigest).Return(fullDirectory, nil)
		replicator.EXPECT().ReplicateMultiple(ctx, digest.NewSetBuilder(2).Add(file1Digest).Add(file2Digest).Build()).Return(nil)

		replicator.EXPECT().ReplicateMultiple(ctx, emptyDirectoryDigest.ToSingletonSet()).Return(nil)
		directoryReader.EXPECT().ReadMessage(ctx, emptyDirectoryDigest).Return(emptyDir, nil)

		replicator.EXPECT().ReplicateMultiple(ctx, treeDigest.ToSingletonSet()).Return(nil)
		treeReader.EXPECT().ReadStream(ctx, treeDigest).Return(buffer.NewProtoBufferFromProto(tree, buffer.UserProvided).ToReader(), nil)
		replicator.EXPECT().ReplicateMultiple(ctx, digest.NewSetBuilder(2).Add(file4Digest).Add(file5Digest).Build()).Return(nil)

		replicator.EXPECT().ReplicateMultiple(ctx, inputRootDirectoryDigest.ToSingletonSet()).Return(nil)
		directoryReader.EXPECT().ReadMessage(ctx, inputRootDirectoryDigest).Return(emptyDir, nil)

		replicator.EXPECT().ReplicateMultiple(ctx, directory2Digest.ToSingletonSet()).Return(nil)
