...
func TestReadCachingBlobAccessGet(t *testing.T) {
	...
	t.Run("Fast", func(t *testing.T) {
		...
		// immediately.
		fastBlobAccess.EXPECT().Get(ctx, digest).Return(buffer.NewValidatedBufferFromByteSlice([]byte("Hello world")))
	...
	t.Run("Slow", func(t *testing.T) {
		...
		fastBlobAccess.EXPECT().Get(ctx, digest).Return(buffer.NewBufferFromError(status.Error(codes.NotFound, "Blob not found")))
		slowBlobAccess.EXPECT().Get(ctx, digest).Return(buffer.NewValidatedBufferFromByteSlice([]byte("Hello world")))
		fastBlobAccess.EXPECT().Put(ctx, digest, gomock.Any()).DoAndReturn(
	...
	t.Run("FastGetError", func(t *testing.T) {
		...
		fastBlobAccess.EXPECT().Get(ctx, digest).Return(buffer.NewBufferFromError(status.Error(codes.Internal, "Disk on fire")))
	...
	t.Run("SlowGetError", func(t *testing.T) {
		...
		fastBlobAccess.EXPECT().Get(ctx, digest).Return(buffer.NewBufferFromError(status.Error(codes.NotFound, "Blob not found")))
		slowBlobAccess.EXPECT().Get(ctx, digest).Return(buffer.NewBufferFromError(status.Error(codes.Internal, "Disk on fire")))
		fastBlobAccess.EXPECT().Put(ctx, digest, gomock.Any()).DoAndReturn(
			func(ctx context.Context, digest *util.Digest, b buffer.Buffer) error {
...
func TestReadCachingBlobAccessPut(t *testing.T) {
	...
	slowBlobAccess.EXPECT().Put(ctx, digest, buffer).Return(nil)

	err := blobAccess.Put(ctx, digest, buffer)
	require.NoError(t, err)
}
