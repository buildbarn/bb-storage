...
func TestChunkListValidatingBlobAccessGetLargeBlob(t *testing.T) {
	ctx := context.Background()
	...
	digestFunction := digest.MustNewFunction("instance", remoteexecution.DigestFunction_SHA256)
	blobData := bytes.Repeat([]byte("test_data_pattern_"), 6000)
	blobDigest := mustComputeDigest(t, digestFunction, blobData)
	...
	splitResponse := msg.(*remoteexecution.SplitBlobResponse)
	require.Greater(t, len(splitResponse.ChunkDigests), 1, "Blob should have been divided into multiple chunks.")

	for _, chunkProto := range splitResponse.ChunkDigests {
		chunkDigest, err := digestFunction.NewDigestFromProto(chunkProto)
		require.NoError(t, err)

		require.Greater(t, fakeCAS.GetTouches(chunkDigest), 0, "Chunk generated by CDC did not have its lifetime renewed.")
		missing, err := fakeCAS.FindMissing(ctx, chunkDigest.ToSingletonSet())
		require.NoError(t, err)
		require.True(t, missing.Empty(), "Chunk generated by CDC was not saved to the CAS.")
	}

	cachedMsg, err := fakeCLS.Get(ctx, blobDigest).ToProto(&remoteexecution.SplitBlobResponse{}, maximumMessageSizeBytes)
	require.NoError(t, err)
	require.Equal(t, len(splitResponse.ChunkDigests), len(cachedMsg.(*remoteexecution.SplitBlobResponse).ChunkDigests))
}
...
func TestChunkListValidatingBlobAccessGetExtendsLifetimes(t *testing.T) {
	...
	digestFunction := digest.MustNewFunction("instance", remoteexecution.DigestFunction_SHA256)
	blobData := bytes.Repeat([]byte("test_data_pattern_"), 6000) // ~108KB
	blobDigest := mustComputeDigest(t, digestFunction, blobData)
	require.NoError(t, fakeCAS.Put(ctx, blobDigest, buffer.NewValidatedBufferFromByteSlice(blobData)))

	// Split the blob to populate the CAS and CLS for this blob.
	msg, err := validatingCLS.Get(ctx, blobDigest).ToProto(&remoteexecution.SplitBlobResponse{}, maximumMessageSizeBytes)
	require.NoError(t, err)
	splitResponse := msg.(*remoteexecution.SplitBlobResponse)
	...
	require.Greater(t, fakeCAS.GetTouches(blobDigest), 0, "Original blob's lifetime was not extended during call to SplitBlob")
	require.Greater(t, fakeCLS.GetTouches(blobDigest), 0, "Original blob's chunk list lifetime was not extended during call to SplitBlob")

	// Every chunk MUST have been touched in the CAS
	for _, chunkProto := range cachedResponse.ChunkDigests {
		chunkDigest, err := digestFunction.NewDigestFromProto(chunkProto)
		require.NoError(t, err)
		require.Greater(t, fakeCAS.GetTouches(chunkDigest), 0, "Chunk's lifetime was not extended during call to SplitBlob")
	}
}
...
func TestChunkListValidatingBlobAccessPutCanonicalization(t *testing.T) {
	...
	blobData := bytes.Repeat([]byte("canonicalization_test_data"), 4000) // ~104KB
	chunk1Data := blobData[:10]
	chunk2Data := blobData[10:]
	...
	composedData, err := fakeCAS.Get(ctx, fullBlobDigest).ToByteSlice(200000)
	require.NoError(t, err)
	require.Equal(t, blobData, composedData)
...
func TestChunkListValidatingBlobAccessPutDigestMismatch(t *testing.T) {
	...
	require.Error(t, err)
	require.Contains(t, err.Error(), "does not compose to blob")
}
...
func TestChunkListValidatingBlobAccessGetMissingBlob(t *testing.T) {
	...
	fakeCAS := newFakeBlobAccess(nil)
	fakeCLS := newFakeBlobAccess(testCDCParams)
	validatingCLS := chunklistvalidating.NewChunkListValidatingBlobAccess(fakeCLS, fakeCAS, 1024*1024)
