...
func TestByteStreamAPI(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "ZSTD", compressor: remoteexecution.Compressor_ZSTD, data: blobData},
	}

	t.Run("WriteAndReadBackBinaryData", func(t *testing.T) {
		for _, test := range tests {
			t.Run(test.name, func(t *testing.T) {
				socketPath := setupCluster(
					t,
					/* minChunkSizeBytes= */ 256<<10,
				)
				_, _, _, bsClient := createClients(t, socketPath)
				// Write blob.
