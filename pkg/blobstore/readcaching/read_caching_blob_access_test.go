...
func TestReadCachingBlobAccessGet(t *testing.T) {
	...
	t.Run("Slow", func(t *testing.T) {
		...
		// attempt to replicate, and then fetch it again.
		gomock.InOrder(
	...
	t.Run("FastError", func(t *testing.T) {
		...
		fastBlobAccess.EXPECT().Get(ctx, blobDigest).Return(nil, status.Error(codes.Internal, "Disk on fire"))

		_, err := blobAccess.Get(ctx, blobDigest)
		testutil.RequireEqualStatus(t, status.Error(codes.Internal, "Fast: Disk on fire"), err)
	})

	t.Run("SlowError", func(t *testing.T) {
		// Replication errors should propagate.
		fastBlobAccess.EXPECT().Get(ctx, blobDigest).Return(nil, status.Error(codes.NotFound, "Blob not found"))
		blobReplicator.EXPECT().ReplicateMultiple(ctx, blobDigest.ToSingletonSet()).Return(status.Error(codes.Internal, "Disk on fire"))
...
func TestReadCachingBlobAccessPut(t *testing.T) {
	...
	slowBlobAccess.EXPECT().Put(ctx, blobDigest, c).Return(nil)

	err := blobAccess.Put(ctx, blobDigest, c)
	require.NoError(t, err)
}
