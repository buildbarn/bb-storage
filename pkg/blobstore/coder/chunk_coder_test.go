...
func TestChunkCoderEncodeRejectsDigestMismatch(t *testing.T) {
	zstdPool := zstd.NewPoolFromConfiguration(nil)
	c := coder.NewChunkCoder(zstdPool)

	_, err := c.Encode(buffer.NewChunk(zstdPool, []byte("Hello")), worldDigest)
	testutil.RequireEqualStatus(t, status.Errorf(codes.InvalidArgument, "Digest mismatch, expected %s, got %s", worldDigest, helloDigest), err)
}
...
func TestChunkCoderEncodeDecodeRoundTrip(t *testing.T) {
	...
	encoded, err := c.Encode(buffer.NewChunk(zstdPool, []byte("Hello")), helloDigest)
...
func TestChunkCoderDecodeRejectsDigestMismatch(t *testing.T) {
	...
	encoded, err := c.Encode(buffer.NewChunk(zstdPool, []byte("World")), worldDigest)
	require.NoError(t, err)
