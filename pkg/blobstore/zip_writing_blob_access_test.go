...
func TestZIPWritingBlobAccess(t *testing.T) {
	...
	t.Run("PutBeforeFinalize", func(t *testing.T) {
		t.Run("Initial", func(t *testing.T) {
			...
			readWriter.EXPECT().WriteAt([]byte{
				...
				0x14, 0x00,
				// Filename.
				0x31, 0x2d, 0x31, 0x38, 0x35, 0x66,
				0x38, 0x64, 0x62, 0x33, 0x32, 0x32,
				0x37, 0x31, 0x66, 0x65, 0x32, 0x35,
				0x66, 0x35, 0x36, 0x31, 0x61, 0x36,
				0x66, 0x63, 0x39, 0x33, 0x38, 0x62,
				0x32, 0x65, 0x32, 0x36, 0x34, 0x33,
				0x30, 0x36, 0x65, 0x63, 0x33, 0x30,
				0x34, 0x65, 0x64, 0x61, 0x35, 0x31,
				0x38, 0x30, 0x30, 0x37, 0x64, 0x31,
				0x37, 0x36, 0x34, 0x38, 0x32, 0x36,
				0x33, 0x38, 0x31, 0x39, 0x36, 0x39,
				0x2d, 0x35,
				...
				0x05, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
			}, int64(0)).Return(118, nil)
			readWriter.EXPECT().WriteAt([]byte("Hello"), int64(118)).Return(5, nil)
		...
		t.Run("Successive", func(t *testing.T) {
			...
			readWriter.EXPECT().WriteAt([]byte{
				...
				0x14, 0x00,
				// Filename.
				0x33, 0x2d, 0x65, 0x62, 0x62, 0x62, 0x62, 0x30,
				0x39, 0x39, 0x65, 0x39, 0x64, 0x32, 0x66, 0x37,
				0x38, 0x39, 0x32, 0x64, 0x39, 0x37, 0x61, 0x62,
				0x33, 0x36, 0x34, 0x30, 0x61, 0x65, 0x38, 0x32,
				0x38, 0x33, 0x2d, 0x39,
				// Tag for the ZIP64 extended information extra field.
				0x01, 0x00,
				// Size of the ZIP64 extended information field.
				...
				0x09, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
			}, int64(123)).Return(86, nil)
			readWriter.EXPECT().WriteAt([]byte("Buildbarn"), int64(209)).Return(9, nil)
	...
	t.Run("Finalize", func(t *testing.T) {
		readWriter.EXPECT().WriteAt([]byte{
			...
			0xff, 0xff, 0xff, 0xff,
			// Filename.
			0x31, 0x2d, 0x31, 0x38, 0x35, 0x66, 0x38, 0x64,
			0x62, 0x33, 0x32, 0x32, 0x37, 0x31, 0x66, 0x65,
			0x32, 0x35, 0x66, 0x35, 0x36, 0x31, 0x61, 0x36,
			0x66, 0x63, 0x39, 0x33, 0x38, 0x62, 0x32, 0x65,
			0x32, 0x36, 0x34, 0x33, 0x30, 0x36, 0x65, 0x63,
			0x33, 0x30, 0x34, 0x65, 0x64, 0x61, 0x35, 0x31,
			0x38, 0x30, 0x30, 0x37, 0x64, 0x31, 0x37, 0x36,
			0x34, 0x38, 0x32, 0x36, 0x33, 0x38, 0x31, 0x39,
			0x36, 0x39, 0x2d, 0x35,
			// Tag for the ZIP64 extended information extra field.
			0x01, 0x00,
			// Size of the ZIP64 extended information field.
			...
			0xff, 0xff, 0xff, 0xff,
			// Filename.
			0x33, 0x2d, 0x65, 0x62, 0x62, 0x62, 0x62, 0x30, 0x39,
			0x39, 0x65, 0x39, 0x64, 0x32, 0x66, 0x37, 0x38, 0x39,
			0x32, 0x64, 0x39, 0x37, 0x61, 0x62, 0x33, 0x36, 0x34,
			0x30, 0x61, 0x65, 0x38, 0x32, 0x38, 0x33, 0x2d, 0x39,
			// Tag for the ZIP64 extended information extra field.
			0x01, 0x00,
			// Size of the ZIP64 extended information field.
			0x18, 0x00,
		...
		require.Equal(t, "1-185f8db32271fe25f561a6fc938b2e264306ec304eda518007d1764826381969-5", files[0].Name)
		...
		require.Equal(t, "3-ebbbb099e9d2f7892d97ab3640ae8283-9", files[1].Name)
