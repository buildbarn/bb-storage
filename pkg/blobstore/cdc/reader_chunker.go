...
func (c *readerChunker) NextChunk() (Chunk, error) {
	...
	// The chunker returns data that aliases an internal buffer which
	// gets reused between calls. Copy it, so that the resulting chunk
	// may be retained for as long as desired, potentially across
	// threads.
	data := bytes.Clone(chunk)

	digestGenerator := c.digestFunction.NewGenerator(int64(len(data)))
