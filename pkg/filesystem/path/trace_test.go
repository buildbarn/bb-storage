	...
	t.Run("WindowsPathValidation", func(t *testing.T) {
		{
			...
			testutil.RequireEqualStatus(t, status.Error(codes.InvalidArgument, "Path component contains a control character or invalid character \"a:\"."), err)
		}
		{
			var p1 *path.Trace
			require.Equal(t, ".", mustGetWindowsString(p1))
			...
			testutil.RequireEqualStatus(t, status.Error(codes.InvalidArgument, "Path component contains a control character or invalid character \"b?\"."), err)
		}
	})
}
