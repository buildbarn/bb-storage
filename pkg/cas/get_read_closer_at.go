...
func GetReadCloserAt(ctx context.Context, chunkBytesReader reader.Reader[[]byte], chunkListFetcher chunklist.Fetcher, params *remoteexecution.RepMaxCdcParams, d digest.Digest, offset int64) (io.ReadCloser, error) {
	...
	offsetManifest := chunklist.ChunkList{
		Digests: manifest.Digests[index:],
		Offsets: manifest.Offsets[index:],
	}
	r := chunklist.NewChunkConcatenatingReader(ctx, offsetManifest, chunkBytesReader)
	if chunkOffset > 0 {
