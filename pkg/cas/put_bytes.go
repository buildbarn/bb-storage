...
func PutBytes(ctx context.Context, zstdPool zstd.Pool, chunkStorage blobstore.BlobAccess[*buffer.Chunk], chunkListStorage blobstore.BlobAccess[chunklist.ChunkList], params *remoteexecution.RepMaxCdcParams, d digest.Digest, data []byte) error {
	if IsSingleChunk(params, d) {
		...
		return chunkStorage.Put(ctx, d, buffer.NewChunk(zstdPool, data))
	}
